(() => {
  ...
  function nextRoundView() {
    ...
    return `<details class="card next-round"><summary>Up next: round ${state.current + 2}</summary>${next.matches.map((m, i) => `<div class="next-court"><span class="court-number">COURT ${courtNumber(i)}</span>${matchView(m)}</div>`).join('')}<p class="help">Resting: ${next.resting.length ? esc(names(next.resting)) : 'Nobody'}</p></details>`;
  }
  ...
  const courtNumber = index => state.config.courtNumbers?.[index] ?? index + 1;
  ...
  function setupView() {
    ...
    for (const id of ['names', 'courts', 'duration', 'game', 'change', 'court-numbers']) document.getElementById(id).addEventListener('input', updatePreview);
    updatePreview();
  }
  function readSetup() {
    return { names: document.getElementById('names').value, courtNumbers: document.getElementById('court-numbers').value, ...Object.fromEntries(['courts', 'duration', 'game', 'change'].map(id => { const value = document.getElementById(id).value; return [id, value === '' ? NaN : Number(value)]; })) };
  }
  ...
  async function createTournament() {
    ...
    try {
      ...
      const config = { courtNumbers: E.courtNumbers(setup.courtNumbers, setup.courts), courts: setup.courts, duration: setup.duration, game: setup.game, change: setup.change };
  ...
  function scoreFields(m, prefix, draft) {
    return matchView(m, prefix, draft);
  }
  function matchView(m, prefix, draft) {
    return `<div class="match-list" aria-label="${esc(names(m.teams[0]) + ' versus ' + names(m.teams[1]))}">${m.teams.map((team, side) => `<div class="pair-row"><div class="team">${team.map(playerLabel).join('<span class="pair-plus" aria-hidden="true">+</span>')}</div>${prefix ? `<label class="pair-score"><span class="sr-only">${esc(names(team))} score</span><input id="${prefix}-${side}" data-score="${prefix}" data-side="${side}" aria-label="${esc(names(team))} score" type="number" inputmode="numeric" min="0" max="99" step="1" placeholder="–" value="${esc(draft?.[side] ?? m.score?.[side] ?? '')}"></label>` : m.score ? `<b class="pair-result" aria-label="${esc(names(team))} score ${m.score[side]}">${m.score[side]}</b>` : ''}</div>`).join('')}</div>`;
  }
  ...
  function playersView() {
    ...
    return `<section class="card"><h2>Players</h2><p class="help">A to Z. Tap a name to see their match history.</p>${players.map(p => {
      ...
      state.rounds.forEach((r, ri) => r.matches.forEach((m, mi) => {
        ...
        history.push(`<article class="player-match" data-round="${ri}" data-court="${mi}"><div class="player-match-heading"><span class="court-number">ROUND ${ri + 1} · COURT ${courtNumber(mi)}</span><span class="match-outcome">${outcome}</span></div>${matchView({ teams: [[p.id, partner], m.teams[1 - side]], score: [own, other] })}</article>`);
      }));

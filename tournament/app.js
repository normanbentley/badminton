(() => {
  ...
  try {
    ...
    if (raw) state = E.validate(JSON.parse(raw));
    const draft = JSON.parse(localStorage.getItem(DRAFT) || 'null');
    if (draft && typeof draft.names === 'string') setup = { ...setup, ...draft };
  ...
  function updatePreview() {
    ...
    } catch (error) { button.disabled = true; preview.textContent = error.message; }
  }
  ...
  app.addEventListener('click', event => {
    ...
    switch (button.dataset.action) {
      case 'create': {
        try { const players = E.parsePlayers(setup.names); const config = { courts: setup.courts, duration: setup.duration, game: setup.game, change: setup.change }; const generated = E.schedule(players, config, Date.now()); state = { version: 1, players, config, ...generated, current: 0, timer: null, drafts: {} }; storageBlocked = false; save(); tab = 'round'; render(); window.scrollTo(0, 0); }
